...
const getAllowedStatuses = (currentStatus: ServiceOrderStatus) => {
  ...
  switch(currentStatus) {
    ...
    case ServiceOrderStatus.DETENIDO:
      allowed = [ServiceOrderStatus.DETENIDO, ServiceOrderStatus.EN_PROCESO, ServiceOrderStatus.LISTO]
      break
    case ServiceOrderStatus.EN_PROCESO:
      allowed = [ServiceOrderStatus.EN_PROCESO, ServiceOrderStatus.DETENIDO, ServiceOrderStatus.LISTO]
      break
...
watch(() => props.serviceOrder, order => {
  if (order) {
...
const updateStatus = async () => {
  ...
  try {
    await router.patch(route('service-orders.update-status', props.serviceOrder.id), {
...
<template>
  <AlertDialog v-model:open="isOpen">
    <AlertDialogContent class="sm:max-w-[425px] mx-auto my-18 p-6 rounded-lg shadow-lg">
      <AlertDialogHeader>
        <AlertDialogTitle class="text-center">Actualizar estado de la orden</AlertDialogTitle>
        <AlertDialogDescription class="text-center text-primary">
          Estás modificando la orden <strong>#{{ serviceOrder?.id }}</strong>.
        </AlertDialogDescription>
      </AlertDialogHeader>

      <form @submit.prevent="updateStatus" class="space-y-4 mt-2">
        <div>
          ...
          <select
            id="status"
            v-model="newStatus"
            class="w-full mt-1 rounded-md border border-input bg-background p-2 text-sm shadow-sm focus:outline-none focus:ring-2 focus:ring-primary"
          ...
          <p v-if="allowedStatuses.length === 1" class="text-sm text-red-500 mt-1">
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction type="submit" :disabled="allowedStatuses.length === 0 || newStatus === serviceOrder?.status">
            Actualizar
          </AlertDialogAction>
        </AlertDialogFooter>
      </form>
